...
## 12-) Planned off-policy experiment protocol and expected diagnostics

The off-policy experiment holds the `r1_zero` prompt, selected learning rate `5e-6`, rollout batch size 256, and other standard-GRPO hyperparameters fixed. The on-policy baseline uses the 256 rollouts in 32 microbatches of 8, accumulates their gradients, and makes one optimizer update. The off-policy variants instead retain the rollout policy's `old_log_probs`, split the 256 rollouts into 32 training batches of 8 (one full group when `group_size=8`), use gradient accumulation 1, and make one optimizer update per batch. Therefore the first batch is nearly on-policy while later batches train on increasingly stale samples from the same rollout batch.

The planned variants are `offpolicy_naive` (`none`), `offpolicy_noclip` (`noclip`), `offpolicy_clip` (token-level `grpo` clipping), and `offpolicy_gspo` (sequence-level geometric-mean `gspo` clipping), each across seeds 0--3. The expected bias--variance ordering is qualitative only: naive reuse has low ratio-induced variance but stale-policy bias; unclipped token reweighting corrects that bias but can have large-ratio variance; clipped token reweighting is a middle ground; and clipped GSPO may be more trajectory-stable because it applies one tempered sequence ratio. These are hypotheses, not results.

For clipped token-level GRPO, `clip_fraction` is the fraction of valid response tokens whose ratio is outside \([1-\epsilon,1+\epsilon]\). For GSPO, it is the fraction of responses whose geometric-mean sequence ratio is outside this range. Increasing clip fraction over the 32 updates would indicate growing rollout staleness. Comparing final validation reward (mean and sample standard deviation across seeds), validation trajectories, and clip-fraction trajectories for GRPO clipping versus GSPO would support the requested stability discussion. Four seeds provide only limited confidence, particularly when error bars overlap.

No off-policy runs were completed because the compute budget was exhausted. Accordingly, no off-policy performance, stability, variance, or clip-fraction claims should be made. The runner also needs a batching change before a valid run: it currently rejects unequal rollout/training batch sizes and does not yet split one 256-rollout batch into 32 separate optimizer updates.

## 13-) Proposed estimator: uncertainty-weighted Dr. GRPO

As a one-change extension of Dr. GRPO, let \(p_i=G^{-1}\sum_j r_{i,j}\) be the empirical success rate for group \(i\). Retain the Dr. GRPO group-mean advantage and multiply it by the bounded uncertainty weight \(w_i=4p_i(1-p_i)\):

\[
A^{\mathrm{UW}}_{i,j}=4p_i(1-p_i)(r_{i,j}-p_i).
\]

All other choices remain Dr. GRPO: the group-mean baseline, no reward-standard-deviation normalization, and constant loss normalization. Thus the proposed estimator changes only the prompt-group weighting. The weight is zero for groups with all identical rewards and reaches one when half the rollouts are correct. It concentrates updates on prompts with competing successful and unsuccessful reasoning paths, where the within-group preference signal is most informative, while downweighting nearly deterministic groups where a rare success or failure can be noisy.

This deliberately changes the effective prompt objective and may bias learning away from very hard prompts with only one correct rollout. The expected trade-off is lower gradient and seed variance versus a possible reduction in final reward if those difficult prompts provide useful learning signal. A valid comparison would run Dr. GRPO and uncertainty-weighted Dr. GRPO with `r1_zero`, learning rate `5e-6`, fixed parameters, and four seeds, reporting validation-reward trajectories, final mean and sample standard deviation, response length, and mean group weight. No such runs were completed because compute was exhausted, so no claim is made that the proposal beats its baseline.
